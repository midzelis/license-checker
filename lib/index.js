...
exports.init = function(options, callback) {
    ...
    read(options.start, opts, function(err, json) {
        ...
        // filter to just the top-level package.json packages
        const packageJson = fs.readFileSync(path.join(options.start, 'package.json'), 'utf-8');
        if (packageJson) {
            ...
            for (const key of Object.keys(data)) {
                const noversion = key.split('@')[0];
                if (!deps.includes(noversion)) {
                    delete data[key];
                }
            }
            if (Object.keys(data).length !== deps.length) {
                console.log(`List of packages doesn't match top level package.json. Expected ${deps.length} but got ${Object.keys(data).length}`);
                const keys = Object.keys(data).map( key => key.split('@')[0]);
                const missing = deps.filter( dep => !keys.includes(dep));
